...
def load_factor_and_market_returns():
    ...
    factor_returns = factor_data["Close"].pct_change().fillna(0)
    market_returns = market_data["Close"].ffill().pct_change().fillna(0)

    return factor_returns, market_returns


def compute_sector_factor_exposures(df_holdings, selected_sector, factor_returns, market_returns):
    sector_tickers = (
    ...
    if isinstance(close_df, pd.Series):
        close_df = close_df.to_frame(name=sector_tickers[0])
